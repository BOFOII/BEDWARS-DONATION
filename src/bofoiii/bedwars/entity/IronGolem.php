...
class IronGolem extends Living implements EntityTeam
{

    public const MAX_TARGET_DISTANCE = 30;
    ...
    {
        return new EntitySizeInfo(2, 0.6);
    }
    ...
    private function updateNameTag(): void
    {
        ...
        $bar = $this->getHealth();
        $this->setNametag($color . "§lDream Defender §r§7[" . $this->lifeTime . "]\n§r§f" . $bar);
    }
    ...
    private function attackTarget(Entity $player): void
    {
    ...
    {
        $rotation = fmod($this->getLocation()->yaw - 90, 360);
        if ($rotation < 0) {
            $rotation += 360.0;
        }
