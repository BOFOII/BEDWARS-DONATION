...
class SetShop extends BaseSubCommand
{
    ...
    protected function prepare(): void
    {
        ...
        $this->setPermission("newbedwars.cmd.setshop");
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if (!isset($args["team"])) {
            ...
            if ($foundTeam == "") {
                ...
                $sender->sendMessage(BedWars::getInstance()->prefix . "§cOr if you set the spawn and it wasn't found automatically try using: /newbedwars setShop <team>");
                ...
                BedWars::getInstance()->getServer()->dispatchCommand($sender, "newbedwars setShop " . $foundTeam);
            }
            return;
        } else {
            if (!in_array($args["team"], $arena->data["teamName"])) {
                ...
                $arena->data["teamShop"][$args["team"]] = Utils::vectorToString(new Vector3(floor($sender->getPosition()->getX()), floor($sender->getPosition()->getY()), floor($sender->getPosition()->getZ())));
                $sender->sendMessage(BedWars::getInstance()->prefix . "§aShop npc set for: " . $arena->data["teamColor"][$args["team"]] . $args["team"]);
                $sender->sendTitle(" ", "§aShop npc set for: " . $arena->data["teamColor"][$args["team"]] . $args["team"], 5, 40, 5);
